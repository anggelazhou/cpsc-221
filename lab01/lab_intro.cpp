...
PNG createSpotlight(PNG image, int centerX, int centerY)
{
  ...
  for (unsigned x = 0; x < image.width(); x++)
  {
    for (unsigned y = 0; y < image.height(); y++)
    {
      ...
      int pixelsOffsetX = centerX - x;
      int pixelsOffsetY = centerY - y;
      double distanceAwayFromCenter = sqrt((pixelsOffsetX * pixelsOffsetX) + (pixelsOffsetY * pixelsOffsetY));
      
      if (distanceAwayFromCenter == 0) {
        pixel->l = 1;
      }
      else if (distanceAwayFromCenter <= 200) {
        pixel->l = 1 * (1 - (distanceAwayFromCenter * 0.5 * 0.01));
      } else {
        pixel->l = 0;
      }
    }
...
PNG ubcify(PNG image)
{
  ...
  for (unsigned x = 0; x < image.width(); x++)
  {
    for (unsigned y = 0; y < image.height(); y++)
    {
      ...
      if (abs(40 - pixel->h) <= abs(210 - pixel->h))
      {
        pixel->h = 40;
      }
      else
...
PNG watermark(PNG firstImage, PNG secondImage)
{
  ...
  for (unsigned x = 0; x < std::min(secondImage.width(), firstImage.width()); x++)
  {
    for (unsigned y = 0; y < std::min(secondImage.height(), firstImage.height()); y++)
    {
      ...
      if (pixel2->l == 1)
      {
        ...
        pixel1->l = std::min(1.0, pixel1->l + 0.2);
      }
    }
