...
void AVLTree<K, V>::rebalance(Node*& subtree)
{
    /** 
     ...
     Node* leftSub = subtree->left;
     Node* rightSub = subtree->right;
     int balance;
     int leftBalance;
     int rightBalance;
     
     balance = height(rightSub) - height(leftSub);

     if (leftSub != NULL) {
         leftBalance = height(leftSub->right) - height(leftSub->left);
         if (balance == -2) {
             if (leftBalance == -1) {
                rotateRight(subtree);
             } else {
     ...
     if (rightSub != NULL) {
         rightBalance = height(rightSub->right) - height(rightSub->left);
         if (balance == 2) {
             if (rightBalance == 1) {
                 rotateLeft(subtree);
             } else {
